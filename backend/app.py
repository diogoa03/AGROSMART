...
app = Flask(__name__)
CORS(app)
socketio = SocketIO(app, cors_allowed_origins="*")  # Crie aqui
logger = setup_logger()

weather_service = WeatherService(socketio)  # Passe o socketio para o serviço
...
def background_weather_updates():
    while True:
        weather_data = weather_service.get_current_weather()
        time.sleep(60)  # Atualiza a cada 5 minutos
